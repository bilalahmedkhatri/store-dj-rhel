...
from .image_utils import resolve_fallback_image
...
def _default_item(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    safe_slug = (
        raw_slug
        or slugify(name)
        or f"item-{abs(hash(name)) % 1000000}"
    )
    image_url = payload.get("image_url") or resolve_fallback_image(safe_slug)
    return {
        "slug": safe_slug,
        "name": name,
        "image_url": image_url,
...
def build_cart_context(request) -> dict[str, Any]:
    ...
    subtotal = 0.0
    for row in cart:
        row_name = row.get("name", "product")
        row_slug = (
            (row.get("slug") or "").strip()
            or slugify(row_name)
            or f"item-{abs(hash(row_name)) % 1000000}"
        )
        qty = int(row.get("quantity", 1))
        price = float(row.get("price", 0))
        total_price = round(price * qty, 2)
        subtotal += total_price
        image_url = row.get("image_url") or resolve_fallback_image(row_slug)
        cart_items.append({
            "slug": row_slug,
            "product": {
